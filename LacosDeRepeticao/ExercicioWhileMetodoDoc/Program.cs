...
namespace ExercicioWhileMetodoDoc
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var texto = FindByWhile(Console.ReadLine());

            Console.WriteLine($"A quantidade de palavras {texto} é: ");
            Console.ReadKey();
        }
        /// <summary>
        /// Método de para encontrar a palavra
        /// </summary>
        /// <param name="wordType">Palavras com tres caracteres</param>
        /// <returns>Retorna a quantidade de palavras</returns>
        private static int FindByWhile(string wordType)
        {
            var textContent = FindText();
            var wordFindCountLength = textContent.Length;
            var lengthText = textContent.Length - (wordFindCountLength - 1);
            var contWord = 0;
            int i = 0;
            while (i < lengthText)
            {
                ...
                {
                    compareWord += textContent[i + b].ToString();
                    b++;
                }
                if (wordType == compareWord)
                    contWord++;
            }
            return contWord;
        }
        ...
        /// <summary>
        /// Método que possui texto muito grande
        /// </summary>
